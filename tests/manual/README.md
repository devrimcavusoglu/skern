...
Tests how AI agents interact with skern — discovery, command chaining, JSON parsing, error handling, and deduplication reasoning. Run before releases and attach the report to PRs.

## Prerequisites

- `skern` built and in PATH (`make build && export PATH="$PWD:$PATH"`)
...
| # | Name | What it tests |
|---|------|---------------|
| 01 | fresh-project | Agent discovers skern, searches before creating |
...
| 11 | autonomous-skill-creation | Agent autonomously discovers skern and creates skills from a project need |
...
tests/manual/
  scenarios/          # Source-controlled scenario definitions
    ...
    ...
  setup.sh            # Creates /tmp/skern-manual-tests/ with pre-populated state
...
- All pre-populated skills use project scope (`.skern/skills/` inside each test dir).
