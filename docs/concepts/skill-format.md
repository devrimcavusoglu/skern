...
| `tags` | No | List of classification tags. Alphanumeric segments joined by hyphens (`code-review`), optionally namespaced as `category:value` with a single colon (`lang:python`, `topic:ci-cd`). Matching is case-insensitive. |
